...
_AVAILABLE_UNITS = ("TurnLeft", "ForwardMover")

# Název úlohy
_TARGET_NAME = f"Testing with {len(_AVAILABLE_UNITS)}"

# Popis úlohy
_TARGET_DESCRIPTION = (
    f"Tento test spočívá v ověření, že program dokáže zjistit názvy všech "
    f"jednotek, kterými je robot osazen. V tomto případě je osazen "
    f"{len(_AVAILABLE_UNITS)} jednotkami.")
...
class UnitNamesTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        # Kontrola, že byl zaznamenán řetězec reprezentující jméno robota
        target.add_task(LoggedMessageInContext(str(_AVAILABLE_UNITS)))
