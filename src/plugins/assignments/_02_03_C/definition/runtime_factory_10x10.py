...
from src.fw.target.task import EndAtPosition, LoggedMessageInContext, \
    AbortedWith, EndedAtCoords, VisitSpecificFieldsTask
...
# Název robota
_ROBOT_NAME = "Héraklei-bot"

# Názvy jednotek k osazení
_AVAILABLE_UNITS = ("ForwardMover", "TurnLeft", "IsWallSensor")

# Název úlohy
_TARGET_NAME = f"10x10"

# Popis úlohy
_TARGET_DESCRIPTION = (
    f"Říká se, že dvakrát nevstoupíš do téže řeky. Platí to i pro rohy? "
    f"Obejdi všechny rohy světa a zastav se na tom, ve kterém jsi začal.")
...
class AroundTheWorldTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        # Kontrola, že se robot posunul na políčko [1, 1]
        target.add_task(EndedAtCoords(1, 1))

        # Kontrola, že byly navštíveny všechny rohy
...
def _get_target_factory() -> "TargetFactory":
    ...
    return AroundTheWorldTestFactory()
...
def _get_world_factory() -> "WorldFactory":
    ...
    return OpenSpaceWorldFactory(12, 12, _get_world_interface_factory(),
