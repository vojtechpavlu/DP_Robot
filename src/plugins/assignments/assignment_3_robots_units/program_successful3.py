...
class Program(AbstractProgram):
    ...
    def run(self, robot: "Robot", log: Callable):
        ...
        log_all_units(robot, log, 0)


def log_all_units(robot: "Robot", log: Callable, iterator: int):
    """Funkce rekurzivně volá sebe sama a pro každou jednotku se pokusí
    vypsat její název."""
    log(robot.units[iterator].name)
    if iterator < len(robot.units) - 1:
        log_all_units(robot, log, iterator + 1)
