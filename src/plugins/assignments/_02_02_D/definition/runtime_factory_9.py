...
# Název robota
_ROBOT_NAME = "Streetwalker"

# Názvy jednotek k osazení
_AVAILABLE_UNITS = ("ForwardMover", )

# Název úlohy
_TARGET_NAME = f"Chodba 9"

# Popis úlohy
_TARGET_DESCRIPTION = (
    f"V této úloze je zadáno, aby robot ušel právě 5 kroků. K tomu je před "
    f"ním vždy dostatečně dlouhá chodba.")
...
class AisleWalkerTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        # Kontrola, že se robot posunul na políčko [6, 1]
        target.add_task(EndedAtCoords(6, 1))
...
def _get_world_factory() -> "WorldFactory":
    ...
    return OpenSpaceWorldFactory(11, 3, _get_world_interface_factory(),
