...
# Název robota
_ROBOT_NAME = "perpebot-mobile"

# Názvy jednotek k osazení
_AVAILABLE_UNITS = ("IsWallSensor", "ForwardMover", )

# Název úlohy
_TARGET_NAME = f"Stěna před ním není"

# Popis úlohy
_TARGET_DESCRIPTION = (
    f"Vědění, že je před robotem stěna, je velmi užitečné. Nyní to zkusme "
    f"aplikovat. Pokud před robotem není stěna, nechť se posune. V obou "
    f"případech by měl zaznamenat svůj postup o posunu (True při posunu a "
    f"False, nedošlo-li k posunu). V tomto případě před robotem stěna není.")
...
class FirstInteractionTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        # Kontrola, že se robot neposunul
        target.add_task(EndAtPosition(1, 1, "SOUTH"))
        target.add_task(LoggedMessageInContext("False"))
...
def _get_spawner_factory() -> "SpawnerFactory":
    ...
    return CoordinatesSpawnerFactory(1, 1, Direction.SOUTH)
...
def _get_world_factory() -> "WorldFactory":
    ...
    return OpenSpaceWorldFactory(4, 4, _get_world_interface_factory(),
