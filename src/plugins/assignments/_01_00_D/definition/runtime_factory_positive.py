...
# Název úlohy
_TARGET_NAME = f"Stěna před ním je"
...
_TARGET_DESCRIPTION = (
    ...
    f"False, nedošlo-li k posunu). V tomto případě před robotem stěna je.")
...
class FirstInteractionTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        target.add_task(EndAtPosition(1, 2, "NORTH"))
        target.add_task(LoggedMessageInContext("True"))
...
def _get_spawner_factory() -> "SpawnerFactory":
    ...
    return CoordinatesSpawnerFactory(1, 1, Direction.NORTH)
