...
_AVAILABLE_UNITS = ("*", )  # ("ForwardMover", )

# Název úlohy
_TARGET_NAME = f"Self-awareness - ForwardMover"

# Popis úlohy
_TARGET_DESCRIPTION = (
    f"Cílem je nyní vybavit robota programem, který mu poskytne možnost "
    f"sebepoznání. Robot je osazen jedinou jednotkou, a to jednotkou názvu "
    f"'ForwardMover'. Úkolem je vypsat do záznamu popis této jednotky.")
...
class SelfAwarenessTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        # Kontrola, že robot vypsal požadovaný text nebo vůbec nějaký text
        target.add_task(LoggedAnythingInContext())
        target.add_task(LoggedMessageInContext(
            "Jednotka, která se stará o posun v aktuálním směru robota",))
...
def _get_target_factory() -> "TargetFactory":
    ...
    return SelfAwarenessTestFactory()
