...
# Stojí před stěnou
_STANDS_AGAINST_THE_WALL = str(False)

# Názvy jednotek k osazení
_AVAILABLE_UNITS = ("IsWallSensor", )

# Název úlohy
_TARGET_NAME = f"Pozitivní případ"

# Popis úlohy
_TARGET_DESCRIPTION = (
    f"Je třeba, aby robot byl schopen rozpoznat stěnu, aby se jí vyhnul. "
    f"Pokud by tak neučinil, při nárazu by se jistě poškodil. Očekávaným "
    f"záznamem je tedy v tomto případě hodnota '{_STANDS_AGAINST_THE_WALL}'")
...
class FirstInteractionTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        # Kontrola, že program vůbec něco zaznamenal
        target.add_task(LoggedAnythingInContext())

        # Kontrola, že program zaznamenal správnou hodnotu
        target.add_task(LoggedMessageInContext(_STANDS_AGAINST_THE_WALL))
