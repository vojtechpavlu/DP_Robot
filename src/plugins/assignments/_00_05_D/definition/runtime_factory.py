...
from src.fw.target.task import EndAtPosition
...
# Název robota
_ROBOT_NAME = "Explorer"

# Názvy jednotek k osazení
_AVAILABLE_UNITS = ("ForwardMover", )

# Název úlohy
_TARGET_NAME = f"First interaction"

# Popis úlohy
_TARGET_DESCRIPTION = (
    f"K čemu je robot, který umí jenom říct své jméno? "
    f"Nyní je za úkol provést svoji první interakci. Robot je vybaven "
    f"jednotkou pro posun dopředu ('ForwardMover') a cílem je, aby se robot "
    f"posunul o políčko před ním. Tam, kam se ještě žádný robot nevydal!")
...
class FirstInteractionTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        # Kontrola, že se robot posunul směrem na východ
