...
from src.fw.target.task import LoggedMessageInContext
...
# Název robota
_ROBOT_NAME = "perceptron"

# Názvy jednotek k osazení
_AVAILABLE_UNITS = ("ForwardMover", "IsWallSensor", "TurnLeft", "HasMark",
                    "MarkReader")

# Název úlohy
_TARGET_NAME = f"Chodba 10"

# Popis úlohy
_TARGET_DESCRIPTION = (
    f"Robot má nyní za úkol poznávat značky položené po chodbě a tyto v "
    f"seznamu abecedně seřazené zaznamenat.")
...
class AisleWalkerTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        target.add_task(LoggedMessageInContext(str(
            ['0A', 'A99', 'B5', 'E0X', 'GAE', 'GTQ', 'RT7', 'RT9', 'VX', 'ZE']
        )))
...
def _get_world_factory() -> "WorldFactory":
    ...
    return FieldPremarker(
        ...
        OpenSpaceWorldFactory(
            12, 3, _get_world_interface_factory(), _get_spawner_factory()),
