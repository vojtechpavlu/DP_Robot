...
def run(robot: Robot, log: Callable, terminate: Callable):
    ...
    # Seznam značek, na které robot narazil
    marks = []

    # Dokud nestojíš před stěnou
    while not robot.get_unit("IsWallSensor").scan():

        # Pokud je značka, zapiš ji
        if robot.get_unit("HasMark").scan():
            marks.append(robot.get_unit("MarkReader").scan())
    ...
    # Značky je požadováno před vrácením ještě seřadit
    marks.sort()

    # Nyní je je možné zaznamenat
    log(marks)

    # Cyklus skončil, takže před stěnou stojím
    terminate("Úspěšně jsem se posunul až k protější stěně a přečetl značky!",
              SUCCESS)
