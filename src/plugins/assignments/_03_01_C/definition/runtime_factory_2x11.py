...
# Název robota
_ROBOT_NAME = "pedometer"

# Názvy jednotek k osazení
_AVAILABLE_UNITS = ("ForwardMover", "IsWallSensor", "TurnLeft")
...
# Popis úlohy
_TARGET_DESCRIPTION = (
    f"Robot nyní musí napočítat, políček svět má. Svět je pravidelný a "
    f"obdélníkový.")
...
class AisleWalkerTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        target.add_task(LoggedMessageInContext("22"))
...
def _get_world_factory() -> "WorldFactory":
    ...
    return OpenSpaceWorldFactory(4, 13, _get_world_interface_factory(),
