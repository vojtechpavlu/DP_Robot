...
def run(robot: Robot, log: Callable, terminate: Callable):
    ...
    # Dokud nestojíš před stěnou, jdi na východ podél jižní stěny světa
    while not is_wall(robot):
        move(robot)
        x += 1
    ...
    # Dokud nestojíš před stěnou, jdi na sever podél východní stěny světa
    while not is_wall(robot):
        move(robot)
        y += 1

    # Zaznamenej součin x a y
    log(x * y)
...
def is_wall(robot: Robot) -> bool:
    """Funkce vrací, zda-li je před robotem stěna či nikoliv."""
    return bool(robot.get_unit("IsWallSensor").scan())
