...
# Název úlohy
_TARGET_NAME = f"Testing with {_ROBOT_NAME}"

# Popis úlohy
_TARGET_DESCRIPTION = (
    f"Tento test spočívá v ověření, že program dokáže zjistit jméno "
    f"robota a to také vrátit. V tomto případě je nastaveno toto jméno "
    f"na '{_ROBOT_NAME}'.")
...
class RobotNameTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        target.add_task(LoggedMessageInContext(_ROBOT_NAME))
