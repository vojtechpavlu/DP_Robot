...
# Název úlohy
_TARGET_NAME = f"Chodba 15"

# Popis úlohy
_TARGET_DESCRIPTION = (
    f"V této úloze je cílem naučit robota dojít samostatně až na konec "
    f"chodby, tedy dokud nestojí před překážkou.")
...
class FirstInteractionTestFactory(TargetFactory):
    def build(self, world: "World", logger: "Logger") -> "Target":
        ...
        target.add_task(EndAtPosition(15, 1, "EAST"))
...
def _get_world_factory() -> "WorldFactory":
    ...
    return OpenSpaceWorldFactory(17, 3, _get_world_interface_factory(),
