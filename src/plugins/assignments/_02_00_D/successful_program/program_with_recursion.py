...
def run(robot: Robot, log: Callable, terminate: Callable):
    ...
    if_wall_end(robot, log, terminate)

# *****************************************************************************
# ------------------------- Definice podprogramů ------------------------------


def if_wall_end(robot: Robot, log: Callable, terminate: Callable):
    """Funkce, která se stará o posun, dokud není před stěnou. Pokud ano,
    ukončí se."""

    # Pokud nestojíš před stěnou
    if not robot.get_unit("IsWallSensor").scan():

        # Posuň se dopředu
        robot.get_unit("ForwardMover").execute()

        # Vyzkoušej to celé znovu
        if_wall_end(robot, log, terminate)

    # Ukončení, neboť program už prostě musí být před stěnou
